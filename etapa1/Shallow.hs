...
plot width height region = intercalate "\n" [
    [if inside (fromIntegral x, fromIntegral y) region then '*' else '.' | x <- [-width..width]]
...
union r1 r2 = \point -> r1 point || r2 point

intersection :: Region -> Region -> Region
intersection r1 r2 = \point -> r1 point && r2 point
...
{-
    *** TODO ***
    ...
    Răspuns: Evaluarea leneșă poate ajuta la eficientizarea procesului de determinare 
    a apartenenței unui punct la regiunea construită prin reuniune , deoarece calculul 
    se face doar pentru cercurile care sunt necesare pentru a determina apartenența.
-}
circles :: Int -> Region
circles n
    | n <= 0    = const False
...
{-
    *** TODO ***
    ...
    Răspuns: Reuniunea infinită va verifica apartenența punctului la fiecare cerc
    din reuniune pana cand gaseste un cerc care contine punctul sau pana cand
    verifica toate cercurile si nu gaseste niciunul care sa contina punctul.
-}
