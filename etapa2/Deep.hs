...
toTransformation (Combine transformations) = foldl (.) id (map toTransformation transformations)
...
fuseTransformations transformations = foldr f [] transformations
  where
    f :: TransformationAST -> [TransformationAST] -> [TransformationAST]
    f (Translation tx ty) (Translation tx' ty' : ts) = Translation (tx + tx') (ty + ty') : ts
    f (Scaling factor) (Scaling factor' : ts) = Scaling (factor * factor') : ts
    f transformation ts = transformation : ts
