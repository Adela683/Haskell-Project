...
instance Show TransformationAST where
    ...
    show (T (Combine transformations)) = "[" ++ (concatMap (\t -> show t ++ ",") (init transformations)) ++ show (last transformations) ++ "]"
...
instance Show RegionAST where
    show region = showIndent 0 region
      where
        showIndent indent (R shape) =
            let ind = replicate (2 * indent) ' '
            in case shape of
                FromPoints points -> ind ++ "FromPoints " ++ show points
                Rectangle w h -> ind ++ "Rectangle " ++ show w ++ " " ++ show h
                Circle r -> ind ++ "Circle " ++ show r
                Complement r -> ind ++ "~\n" ++ showIndent (indent + 1) r
                Union r1 r2 -> ind ++ "+\n" ++ showIndent (indent + 1) r1 ++ "\n" ++ showIndent (indent + 1) r2
                Intersection r1 r2 -> ind ++ "*\n" ++ showIndent (indent + 1) r1 ++ "\n" ++ showIndent (indent + 1) r2
                Transform t r -> ind ++ show t ++ "\n" ++ showIndent (indent + 1) r
